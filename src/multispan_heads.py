...
from src.conditional_random_field import ConditionalRandomField, allowed_transitions
...
class MultiSpanHead(Module):
    def __init__(self,
                 ...
                 dropout: float = 0.1,
                 bio_or_io: str = 'bio') -> None:
        super(MultiSpanHead, self).__init__()
        self.bert_dim = bert_dim
        self.dropout = dropout
        self.bio_or_io = 'bio'
        self.predictor = predictor or MultiSpanHead.default_predictor(self.bert_dim, self.dropout, self.bio_or_io)
    ...
    @staticmethod
    def default_predictor(bert_dim, dropout, bio_or_io):
        if bio_or_io == 'bio':
            return ff(bert_dim, bert_dim, 3, dropout)
        else:
            return ff(bert_dim, bert_dim, 2, dropout)

    @staticmethod
    def decode_spans_from_bio_tags(tags, question_passage_tokens, passage_text, question_text):
        spans_tokens = []
        prev = 0  # 0 = O
    ...
    @staticmethod
    def decode_spans_from_io_tags(tags, question_passage_tokens, passage_text, question_text):
        spans_tokens = []
        prev = 0  # 0 = O
...
class SimpleBIO(MultiSpanHead):
    def __init__(self, bert_dim: int, predictor: Module = None, dropout_prob: float = 0.1, bio_or_io: str = 'bio') -> None:
        super(SimpleBIO, self).__init__(bert_dim, predictor, dropout_prob, bio_or_io)
    ...
    def log_likelihood(self, gold_labels, log_probs, seq_mask, is_bio_mask, **kwargs):
        ...
        log_marginal_likelihood_for_multispan = \
            replace_masked_values(log_marginal_likelihood_for_multispan, is_bio_mask, 0) # PRE: -1e7
    ...
    def prediction(self, log_probs, logits, qp_tokens, p_text, q_text, mask):
        ...
        return MultiSpanHead.decode_spans_from_bio_tags(predicted_tags, qp_tokens, p_text, q_text)


class SimpleIO(MultiSpanHead):
    def __init__(self, bert_dim: int, predictor: Module = None, dropout_prob: float = 0.1, bio_or_io: str = 'io') -> None:
        super(SimpleIO, self).__init__(bert_dim, predictor, dropout_prob, bio_or_io)
    ...
    def log_likelihood(self, gold_labels, log_probs, seq_mask, is_io_mask, **kwargs):
        ...
        # For questions without spans, we set their log probabilities to be very small negative value
        log_marginal_likelihood_for_multispan = \
            replace_masked_values(log_marginal_likelihood_for_multispan, is_io_mask, 0) # PRE: -1e7
    ...
    def prediction(self, log_probs, logits, qp_tokens, p_text, q_text, mask):
        ...
        return MultiSpanHead.decode_spans_from_io_tags(predicted_tags, qp_tokens, p_text, q_text)


class CRFLossBIO(MultiSpanHead):

    def __init__(self, bert_dim: int, predictor: Module = None, dropout_prob: float = 0.1, bio_or_io: str = 'bio') -> None:
        super(CRFLossBIO, self).__init__(bert_dim, predictor, dropout_prob, bio_or_io)

        # create crf for tag decoding
        self.crf = default_crf_bio()
    ...
    def prediction(self, log_probs, logits, qp_tokens, p_text, q_text, mask):
        ...
        return MultiSpanHead.decode_spans_from_bio_tags(predicted_tags[0], qp_tokens, p_text, q_text)


class CRFLossIO(MultiSpanHead):

    def __init__(self, bert_dim: int, predictor: Module = None, dropout_prob: float = 0.1, bio_or_io: str = 'io') -> None:
        super(CRFLossIO, self).__init__(bert_dim, predictor, dropout_prob, bio_or_io)

        # create crf for tag decoding
        self.crf = default_crf_io()
    ...
    def log_likelihood(self, gold_labels, log_probs, seq_mask, is_io_mask, **kwargs):
        ...
        if gold_labels is not None:
            ...
            log_likelihood = replace_masked_values(log_likelihood, is_io_mask, -1e7)
    ...
    def prediction(self, log_probs, logits, qp_tokens, p_text, q_text, mask):
        ...
        return MultiSpanHead.decode_spans_from_io_tags(predicted_tags[0], qp_tokens, p_text, q_text)


class FlexibleLossBIO(MultiSpanHead):

    def __init__(self, bert_dim: int, generation_top_k, prediction_beam_size, predictor: Module = None, dropout_prob: float = 0.1, use_crf=False, bio_or_io='bio') -> None:
        super(FlexibleLossBIO, self).__init__(bert_dim, predictor, dropout_prob, bio_or_io)
        
        self.crf = default_crf_bio() # SHOULD BE REMOVED, USED ONLY TO LOAD OLDER MODELS

        self._use_crf = use_crf
        if use_crf:
            self._crf = default_crf_bio()
        else:
...
class FlexibleLossIO(MultiSpanHead):

    def __init__(self, bert_dim: int, generation_top_k, prediction_beam_size, predictor: Module = None, dropout_prob: float = 0.1, use_crf=False, bio_or_io='io') -> None:
        super(FlexibleLossIO, self).__init__(bert_dim, predictor, dropout_prob, bio_or_io)
        
        self.crf = default_crf_io() # SHOULD BE REMOVED, USED ONLY TO LOAD OLDER MODELS

        self._use_crf = use_crf
        if use_crf:
            self._crf = default_crf_io()
        else:
            self._start_index, self._end_index = 3, 4
            self._per_node_beam_size = 3
            self._generation_top_k = generation_top_k
    ...
    def log_likelihood(self, answer_as_text_to_disjoint_ios, answer_as_list_of_ios, span_io_labels, log_probs, 
                        logits, seq_mask, wordpiece_mask, is_io_mask, **kwargs):
        # answer_as_text_to_disjoint_ios - Shape: (batch_size, # of text answers, # of spans a for text answer, seq_length)
        # answer_as_list_of_ios - Shape: (batch_size, # of correct sequences, seq_length)
        # log_probs - Shape: (batch_size, seq_length, 2)
        # seq_mask - Shape: (batch_size, seq_length)
        ...
        if self._use_crf:
            ...
            with torch.no_grad():
                answer_as_list_of_ios = answer_as_list_of_ios * seq_mask.unsqueeze(1)
                if answer_as_text_to_disjoint_ios.sum() > 0:
                    full_io = span_io_labels
                    ...
                    if self._generation_top_k > 0:
                        ...
                        generated_list_of_ios = self._filter_correct_predictions(most_likely_predictions, answer_as_text_to_disjoint_ios, full_io)

                        is_pregenerated_answer_format_mask = (answer_as_list_of_ios.sum((1)) > 0).unsqueeze(-1).unsqueeze(-1).long()
                        list_of_ios = torch.cat((answer_as_list_of_ios, (generated_list_of_ios * (1 - is_pregenerated_answer_format_mask))), dim=1)

                        list_of_ios = self._add_full_io(list_of_ios, full_io)
                    else:
                        is_pregenerated_answer_format_mask = (answer_as_list_of_ios.sum((1)) > 0).long()
                        list_of_ios = torch.cat((answer_as_list_of_ios, (full_io * (1 - is_pregenerated_answer_format_mask).unsqueeze(-1)).unsqueeze(1)), dim=1)
                else:
                    list_of_ios = answer_as_list_of_ios

        ### Calculate log-likelihood from list_of_ios
        if self._use_crf:
            raise NotImplementedError
        else:
            log_marginal_likelihood_for_multispan = self._get_combined_likelihood(list_of_ios, log_probs)
    ...
    def _filter_correct_predictions(self, predictions, answer_as_text_to_disjoint_ios, full_io):
        texts_count = answer_as_text_to_disjoint_ios.size()[1]
        spans_count = answer_as_text_to_disjoint_ios.size()[2]
        predictions_count = predictions.size()[1]

        expanded_predictions = predictions.unsqueeze(2).unsqueeze(2).repeat(1, 1, texts_count, spans_count, 1)
        expanded_answer_as_text_to_disjoint_ios = answer_as_text_to_disjoint_ios.unsqueeze(1)
        expanded_full_io = full_io.unsqueeze(1).unsqueeze(-2).unsqueeze(-2)

        disjoint_intersections = (expanded_predictions == expanded_answer_as_text_to_disjoint_ios) & (expanded_answer_as_text_to_disjoint_ios != 0)
        some_intersection = disjoint_intersections.sum(-1) > 0
        only_full_intersections = (((expanded_answer_as_text_to_disjoint_ios != 0) - disjoint_intersections).sum(-1) == 0) & (expanded_answer_as_text_to_disjoint_ios.sum(-1) > 0)
        valid_texts = (((some_intersection ^ only_full_intersections)).sum(-1) == 0) & (only_full_intersections.sum(-1) > 0)
        correct_mask = ((valid_texts == 1).prod(-1) != 0).long()
        correct_mask &= (((expanded_full_io != expanded_predictions) & (expanded_predictions != 0)).sum((-1, -2)) == 0).long()

        return predictions * correct_mask.unsqueeze(-1)

    def _add_full_io(self, correct_most_likely_predictions, full_io):
        predictions_count = correct_most_likely_predictions.size()[1]

        not_added = ((full_io.unsqueeze(1) == correct_most_likely_predictions).prod(-1).sum(-1) == 0).long()

        return torch.cat((correct_most_likely_predictions, (full_io * not_added.unsqueeze(-1)).unsqueeze(1)), dim=1)

    def _get_combined_likelihood(self, answer_as_list_of_ios, log_probs):
        # answer_as_list_of_ios - Shape: (batch_size, # of correct sequences, seq_length)
        # log_probs - Shape: (batch_size, seq_length, 2)

        # Shape: (batch_size, # of correct sequences, seq_length, 2)
        # duplicate log_probs for each gold ios sequence
        expanded_log_probs = log_probs.unsqueeze(1).expand(-1, answer_as_list_of_ios.size()[1], -1, -1)
        
        # get the log-likelihood per each sequence index
        # Shape: (batch_size, # of correct sequences, seq_length)
        log_likelihoods = \
            torch.gather(expanded_log_probs, dim=-1, index=answer_as_list_of_ios.unsqueeze(-1)).squeeze(-1)

        # Shape: (batch_size, # of correct sequences)
        correct_sequences_pad_mask = (answer_as_list_of_ios.sum(-1) > 0).long()
    ...
    def take_step(self, last_predictions: torch.Tensor,
        ...
        # mask illegal IO transitions
        transitions_mask = torch.cat((torch.ones_like(class_log_probabilities[:,:2]), torch.zeros_like(class_log_probabilities[:,-1:])), dim=-1).byte()
        transitions_mask[:,1] &= ((last_predictions == 1))
        transitions_mask[:,1:2] &= ((class_log_probabilities[:,:2] == 0.0).sum(-1) != 2).unsqueeze(-1).repeat(1)

        # assuming the wordpiece mask doesn't intersect with the other masks (pad, cls/sep)
        transitions_mask[:,1] |= is_wordpiece & ((last_predictions == 1))
...
multispan_heads_mapping = {'simple_bio': SimpleBIO, 'simple_io': SimpleIO, 'crf_loss_bio': CRFLossBIO, 'crf_loss_io': CRFLossIO, 'flexible_loss_bio': FlexibleLossBIO, 'flexible_loss_io': FlexibleLossIO}
...
def default_crf_bio() -> ConditionalRandomField:
...
def default_crf_io() -> ConditionalRandomField:
    include_start_end_transitions = True
    constraints = allowed_transitions('IO', {0: 'O', 1: 'I'})
    return ConditionalRandomField(2, constraints, include_start_end_transitions)
