...
def _align_bags(predicted: List[Set[str]], gold: List[Set[str]], metric: str) -> List[float]:
    """
    ...
    for gold_index, gold_item in enumerate(gold):
        for pred_index, pred_item in enumerate(predicted):
            if _match_numbers_if_present(gold_item, pred_item):
                f1, precision, recall= _compute_f1_p_r(pred_item, gold_item)
                if metric == "f1":
                    scores[gold_index, pred_index] = f1
                elif metric == "precision":
                    scores[gold_index, pred_index] = precision
                elif metric == "recall":
                    scores[gold_index, pred_index] = recall
                else:
...
def _compute_f1_p_r(predicted_bag: Set[str], gold_bag: Set[str]) -> float:
...
def get_metrics(
    ...
    if set(predicted_bags[0]) == set(gold_bags[0]) and len(predicted_bags[0]) == len(gold_bags[0]):
        exact_match = 1.0
        if len(gold_bags[0]) == 0:
            return 1.0, 1.0, 1.0, 1.0
    else:
        exact_match = 0.0

    f1_per_bag = _align_bags(predicted_bags[1], gold_bags[1], "f1")
    f1 = np.mean(f1_per_bag)
    f1 = round(f1, 2)

    precision_per_bag = _align_bags(predicted_bags[1], gold_bags[1], "precision")
    precision = np.mean(precision_per_bag)
    precision = round(precision, 2)

    recall_per_bag = _align_bags(predicted_bags[1], gold_bags[1], "recall")
        ...
        for qa_pair in annotation["qa_pairs"]:
            ...
            if query_id in predicted_answers:
                ...
                for answer in candidate_answers:
                    ...
                    if len(gold_answer) == 0 or (len(gold_answer) > 0 and gold_answer[0].strip() != ""):
